...
import { ContactEmailDraftModal } from './ContactEmailDraftModal';
...
export const ContactDetailModal: React.FC<ContactDetailModalProps> = ({
  ...
  return createPortal(
    ...
    >
        ...
        <div className="space-y-4 overflow-y-auto pr-1 flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            <button
              ...
              title="Vorschau des E-Mail-Entwurfs vor dem Download anzeigen"
        ...
        <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-2 py-1 text-[11px] font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg transition cursor-pointer flex items-center gap-1"
              title={t('contact.btn_delete', currentLang, 'Delete Contact')}
            >
              <Trash2 className="w-3 h-3" />
              <span>{t('contact.btn_delete', currentLang, 'Delete Contact')}</span>
            </button>
          </div>
      ...
      {/* Delete Confirmation Modal (In-App Dialog without window.confirm) */}
      {isDeleteConfirmOpen && (
        <div 
          id="contact-delete-confirm-modal-backdrop"
        ...
        >
          <div 
            id="contact-delete-confirm-dialog"
            className="bg-white dark:bg-slate-900 rounded-3xl border border-rose-200 dark:border-rose-900/60 shadow-2xl max-w-md w-full p-6 space-y-4 animate-scale-in"
          >
            <div className="flex items-center gap-3 text-rose-600 dark:text-rose-400">
              <div className="p-2.5 rounded-2xl bg-rose-100 dark:bg-rose-950/60 shrink-0">
                <AlertTriangle className="w-6 h-6 text-rose-600 dark:text-rose-400" />
              </div>
              <div>
                <h3 className="font-bold text-base text-slate-900 dark:text-white">
                  {t('contact.delete_modal_title', currentLang, 'Kontakt löschen')}
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">
                  {displayName} {contact.id ? `(#${contact.id})` : ''}
                </p>
              </div>
            </div>

            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              {t('contact.delete_modal_text', currentLang, 'Möchten Sie diesen Kontakt wirklich unwiderruflich aus der lokalen Datenbank löschen? Alle Stammdaten dieses Kontakts werden entfernt.')}
            </p>
            ...
            {contactInvoices.length > 0 && (
              <div className="p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-xl text-xs text-amber-800 dark:text-amber-300 flex items-start gap-2">
                ...
                <span>
                  {t('contact.delete_modal_invoices_warning', currentLang, 'Hinweis: Zu diesem Kontakt existieren bereits Rechnungen.')}
                </span>
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl text-xs font-bold bg-rose-600 hover:bg-rose-700 text-white shadow-md transition flex items-center gap-1.5 cursor-pointer active:scale-95"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>{t('contact.btn_confirm_delete', currentLang, 'Kontakt löschen')}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* EML Email Draft Preview Modal */}
      <ContactEmailDraftModal
        isOpen={isEmailDraftModalOpen}
        onClose={() => setIsEmailDraftModalOpen(false)}
        contact={contact}
        company={company}
      />
    </div>,
