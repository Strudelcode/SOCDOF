...
export const DiscordFeedbackModal: React.FC<DiscordFeedbackModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'bug' | 'idea' | 'history'>(initialType);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      window.addEventListener('socdof:discord-reports-changed', handleReportsSynced);

      return () => {
        clearInterval(interval);
        window.removeEventListener('socdof:discord-reports-synced', handleReportsSynced);
        window.removeEventListener('socdof:discord-reports-changed', handleReportsSynced);
      };
    }
  }, [isOpen, initialType, performSync]);
  ...
  return (
    <div 
      className="fixed inset-0 z-[99999] flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm animate-fade-in select-text"
      onClick={onClose}
    >
      <div 
        onClick={e => e.stopPropagation()}
        className="w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-scale-up"
      >
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between shrink-0 bg-slate-50/50 dark:bg-slate-850/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#5865F2] text-white flex items-center justify-center shadow-md shrink-0">
        ...
        {/* Tab Switcher */}
        <div className="p-3 bg-slate-100/80 dark:bg-slate-800/80 border-b border-slate-200/80 dark:border-slate-700/60 shrink-0">
          <div className="grid grid-cols-3 gap-1 bg-white/70 dark:bg-slate-900/70 p-1 rounded-2xl">
            <button
              type="button"
              onClick={() => {
                sounds.playClick();
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-4">
          ...
          {activeTab === 'history' ? (
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between p-2.5 rounded-2xl bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/40 text-[11px]">
                <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={isSyncing}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-white dark:bg-slate-800 border border-indigo-200 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 font-bold transition cursor-pointer"
                >
                  <RefreshCw className={`w-3 h-3 ${isSyncing ? 'animate-spin' : ''}`} />
                  <span>{isSyncing ? t('feedback.syncing', lang, 'Prüfe Discord...') : t('feedback.sync_now', lang, 'Aktualisieren')}</span>
                </button>
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {submittedReports.length > 0 && (
                    <button
                      ...
                      className="text-xs text-rose-500 hover:text-rose-600 hover:underline cursor-pointer flex items-center gap-1"
                    >
              ...
              {filteredReports.length === 0 ? (
                ...
                <div className="space-y-2.5">
                  {filteredReports.map(rep => (
                    <div
                      key={rep.id}
                      className={`p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-850 border space-y-2.5 transition ${
                        rep.status === 'resolved'
                    ...
                    >
                        ...
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          {rep.status === 'rejected' ? (
                            ...
                            <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300 border border-amber-300 dark:border-amber-800 flex items-center gap-1">
                              ...
                              <span>{t('feedback.status_in_progress', lang, '🔨 Fix in Bearbeitung')}</span>
                            </span>
                            ...
                            <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-slate-200 text-slate-700 border-slate-300 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700 flex items-center gap-1">
                              <Clock className="w-3 h-3 text-slate-500 dark:text-slate-400" />
                              <span>{t('feedback.status_pending', lang, '⏳ Neue Einreichung')}</span>
                            </span>
                          )}

                          {/* Message count replies button */}
                          ...
                          {(rep.isArchived || rep.isLocked) && (
                            <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
                              🔒 {t('feedback.discord_archived', lang, 'Archiviert')}
                            </span>
                          )}
                      ...
                      {rep.appliedTags && rep.appliedTags.length > 0 && (
                        <div className="flex flex-wrap items-center gap-1.5 p-1.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
                          <span className="text-[9.5px] font-bold text-slate-400 flex items-center gap-1">
                            <Tag className="w-3 h-3 text-[#5865F2]" />
                            <span>{t('feedback.discord_tags_label', lang, 'Tags:')}</span>
                          </span>
                          {rep.appliedTags.map((tag, idx) => {
                            ...
                            return (
                              <span
                                key={idx}
                                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-[#5865F2]/10 dark:bg-[#5865F2]/20 border border-[#5865F2]/30 text-[#5865F2] dark:text-indigo-300 shadow-2xs"
                              >
                                {customEmojiUrl ? (
                      ...
                      <div className="p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-[11px] font-mono text-slate-700 dark:text-slate-300 whitespace-pre-wrap">
                        {rep.description}
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="p-1 text-slate-400 hover:text-rose-500 transition cursor-pointer rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/30"
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 space-y-3.5 shadow-xs">
                  ...
                  <div>
                    ...
                    {!bugLocation ? (
                      ...
                      <div className="flex items-center justify-between p-2.5 rounded-2xl bg-orange-50/70 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-800">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 rounded-xl bg-white dark:bg-slate-800 border border-orange-300 dark:border-orange-800 text-orange-600 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-900/40 text-xs font-bold transition cursor-pointer shrink-0 ml-2"
                        >
                    ...
                    {(bugLocation.includes('Sonstiges') || bugLocation.includes('Other') || bugLocation.includes('Autre') || bugLocation.includes('Otro')) && (
                      <div className="mt-2 animate-fade-in">
                        <input
                          ...
                          className="w-full px-3 py-1.5 bg-orange-50/40 dark:bg-orange-950/20 border border-orange-300 dark:border-orange-800/60 rounded-xl text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500"
                          required
                        />
                      </div>
                    )}
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 space-y-3.5 shadow-xs">
                  ...
                  <div>
                    ...
                    {!ideaCategory ? (
                      ...
                      <div className="flex items-center justify-between p-2.5 rounded-2xl bg-indigo-50/70 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800">
                        <div className="flex items-center gap-2 min-w-0">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 rounded-xl bg-white dark:bg-slate-800 border border-indigo-300 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 text-xs font-bold transition cursor-pointer shrink-0 ml-2"
                        >
                    ...
                    {(ideaCategory.includes('Sonstiges') || ideaCategory.includes('Other') || ideaCategory.includes('Autre') || ideaCategory.includes('Otro')) && (
                      <div className="mt-2 animate-fade-in">
                        <input
                          ...
                          className="w-full px-3 py-1.5 bg-indigo-50/40 dark:bg-indigo-950/20 border border-indigo-300 dark:border-indigo-800/60 rounded-xl text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          required
                        />
                      </div>
                    )}
