...
export const ContactsModule: React.FC<ContactsModuleProps> = ({
  ...
  return (
    <div className="space-y-6 max-w-7xl animate-fade-in">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {contacts.length} Einträge (Kunden & Lieferanten)
            </p>
          </div>
        </div>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${c.type === 'customer' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-400' : c.type === 'vendor' ? 'bg-indigo-100 text-indigo-800 dark:bg-indigo-950/60 dark:text-indigo-400' : 'bg-amber-100 text-amber-800'}`}>
                        {c.type === 'customer' ? 'Kunde' : c.type === 'vendor' ? 'Lieferant' : 'Partner'}
                      </span>
                    </div>
        ...
        {selectedContact && (
          <div className="lg:col-span-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 space-y-5 animate-fade-in">
            <div className="flex items-start justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {selectedContact.company || 'Privatkunde / Einzelperson'}
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60">
              <div>
                <span className="text-[10px] font-bold text-slate-400 uppercase">Bezahlt Gesamt</span>
              ...
              <div>
                <span className="text-[10px] font-bold text-slate-400 uppercase">Offene Forderungen</span>
            ...
            <div className="space-y-2.5 text-xs text-slate-700 dark:text-slate-300">
              <div>
                <span className="text-[10px] text-slate-400 block font-semibold">E-Mail</span>
              ...
              {selectedContact.phone && (
                <div>
                  <span className="text-[10px] text-slate-400 block font-semibold">Telefon</span>
              ...
              {selectedContact.street && (
                <div>
                  <span className="text-[10px] text-slate-400 block font-semibold">Anschrift</span>
              ...
              {selectedContact.taxId && (
                <div>
                  <span className="text-[10px] text-slate-400 block font-semibold">USt-IdNr</span>
              ...
              {selectedContact.notes && (
                <div>
                  <span className="text-[10px] text-slate-400 block font-semibold">Notizen</span>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 max-w-lg w-full p-6 shadow-2xl space-y-4 animate-fade-in">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <h3 className="font-bold text-sm text-slate-900 dark:text-white">
                {editingContact.id ? 'Kontakt bearbeiten' : 'Neuen Kontakt anlegen'}
              </h3>
              <button
      ...
      {isBatchModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/75 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 max-w-3xl w-full p-6 shadow-2xl space-y-4 animate-fade-in">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2">
                <Layers className="w-5 h-5 text-emerald-600" />
                <h3 className="font-bold text-sm text-slate-900 dark:text-white">
                  Mehrere Kontakte gleichzeitig anlegen
                </h3>
              </div>
              <button
      ...
      {isImportModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/75 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 max-w-2xl w-full p-6 shadow-2xl space-y-4 animate-fade-in">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <div>
                <h3 className="font-bold text-sm text-slate-900 dark:text-white">
                  Kontakte importieren: {importFileName}
                </h3>
                <p className="text-xs text-slate-500">
                  {importedPreview.length} gültige Kontakte erkannt
                </p>
              </div>
              <button
            ...
            <div className="max-h-60 overflow-y-auto border border-slate-200 dark:border-slate-800 rounded-xl">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 border-b border-slate-200 dark:border-slate-700">
                  <tr>
                    ...
                    <th className="p-2.5 font-bold">Firma</th>
                    <th className="p-2.5 font-bold">E-Mail</th>
                    <th className="p-2.5 font-bold">Telefon</th>
                  </tr>
                </thead>
              ...
              {importedPreview.length > 10 && (
                <div className="p-2 text-center text-xs text-slate-400 bg-slate-50 dark:bg-slate-800/30">
                  ... und {importedPreview.length - 10} weitere Kontakte
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              >
                {importedPreview.length} Kontakte in Datenbank übernehmen
              </button>
            </div>
          </div>
        </div>
