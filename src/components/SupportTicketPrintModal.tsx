...
export const SupportTicketPrintModal: React.FC<SupportTicketPrintModalProps> = ({
  ...
  const handleCopyTextReport = () => {
    sounds.playSuccess();
    let text = `=================================================\n`;
    text += `KUNDENDIENST- & SERVICEBERICHT / SERVICE REPORT\n`;
    text += `Ticket-Nr.: ${ticket.ticketNumber}\n`;
    text += `Datum: ${formatDate(ticket.createdAt)}\n`;
    text += `Status: ${getStatusLabel(ticket.status)}\n`;
    text += `Kunde: ${ticket.contact_name || '-'}${ticket.contact_company ? ` (${ticket.contact_company})` : ''}\n`;
    text += `Titel: ${ticket.title}\n`;
    text += `=================================================\n\n`;
    if (ticket.description) {
      text += `FEHLERBESCHREIBUNG / AUFTRAG:\n${ticket.description}\n\n`;
    }
    if (workItems.length > 0) {
      text += `SERVICE-POSITIONEN & ARBEITSSCHRITTE:\n`;
    ...
    if (ticket.timesheets && ticket.timesheets.length > 0) {
      ...
      ticket.timesheets.forEach(ts => {
        text += `- ${formatDate(ts.date)} | ${ts.staff || 'Service'}: ${ts.description} (${ts.hours} h)\n`;
      });
      text += `Gesamtstunden: ${totalWorkHours.toFixed(1)} h\n\n`;
    }
    text += `ABRECHNUNG:\n`;
    text += `Arbeitszeit: ${totalWorkHours.toFixed(1)} h à ${formatCurrency(hourlyRate)} = ${formatCurrency(hoursAmount)}\n`;
    if (workItemsAmount > 0) {
      text += `Service-Positionen: ${formatCurrency(workItemsAmount)}\n`;
    }
    text += `Nettobetrag: ${formatCurrency(totalSubtotal)}\n`;
    text += `Gesamtbetrag (inkl. ${taxRate}% MwSt.): ${formatCurrency(totalGross)}\n`;
    text += `=================================================\n`;
        ...
        <div className="no-print p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/90 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Bericht als Text kopieren"
            >
          ...
          <div className="relative bg-white text-slate-900 w-full max-w-[210mm] min-h-[297mm] p-8 sm:p-12 shadow-lg rounded-xl print:shadow-none print:p-0 print:m-0 flex flex-col justify-between border border-slate-200/80 print:border-none font-sans text-xs">
              ...
              <div className="flex justify-between items-start border-b-2 border-slate-900 pb-5 mb-6">
                <div>
                  <h1 className="text-xl font-black text-slate-900 tracking-tight uppercase">
                    {company.name || 'Kundendienst & Service'}
                  </h1>
                  {company.subtitle && (
                ...
                <div className="mb-6">
                  <h3 className="font-bold text-xs uppercase tracking-wider text-slate-800 mb-2 flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-normal text-slate-500">
                      {workItems.filter(w => w.isCompleted).length} von {workItems.length} erledigt
                    </span>
                  </h3>
                    ...
                    <tbody className="divide-y divide-slate-200">
                      {ticket.timesheets.map((ts) => (
                        <tr key={ts.id}>
                          ...
                          <td className="py-2 px-3 text-slate-800 font-medium">{ts.staff || 'Techniker'}</td>
              ...
              <div className="flex justify-end mb-8">
                <div className="w-72 bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-1.5 text-xs">
                  <div className="flex justify-between">
                    <span className="text-slate-600">Arbeitszeit ({totalWorkHours.toFixed(1)} h à {formatCurrency(hourlyRate)}):</span>
                    <span className="font-mono font-medium">{formatCurrency(hoursAmount)}</span>
                  </div>
                  {workItemsAmount > 0 && (
                    <div className="flex justify-between">
                      <span className="text-slate-600">Service-Positionen / Pauschalen:</span>
                  ...
                  <div className="flex justify-between pt-1.5 border-t border-slate-200 font-semibold">
                    <span className="text-slate-800">Nettobetrag:</span>
            ...
            <div className="pt-6 border-t border-slate-200 mt-auto">
              ...
              <div className="mt-8 pt-4 border-t border-slate-100 flex justify-between text-[9px] text-slate-400">
                ...
                {company.tax_number && <span>Steuer-Nr.: {company.tax_number}</span>}
              </div>
            </div>
