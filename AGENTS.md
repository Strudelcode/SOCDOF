...
4. **Discord Webhook & Release Changelog Formatting**:
   - Every GitHub Release triggers `.github/workflows/discord_release.yml` to automatically broadcast formatted changelogs to the Discord community.
   - Always ensure release notes in `versions/V<Major>.md` and GitHub releases are structured with clear markdown headings, bulleted feature highlights, and concise summaries so Discord embeds render cleanly.
