...
## 🌟 Version 22.6.9 (Kunden-Stundensatz, Kontaktbearbeitung & Support-Automatisierung)
- 🚀 **Neu**: Individueller Standard-Stundensatz pro Kunde – im Kundenprofil kann nun ein spezifischer Standard-Stundensatz (z. B. `95,00 € / Std.`) hinterlegt werden.
- 🚀 **Neu**: Vollwertiger Kunden-Bearbeiten-Dialog (`ContactEditModal`) – Kunden können direkt und umfassend mit allen Stammdaten, Notizen und Stundensätzen bearbeitet werden.
- 🔄 **Verbessert**: Automatische Stundensatz-Übernahme im Support- & Service-Modul – bei Auswahl oder Zuweisung eines Kunden wird dessen individueller Stundensatz automatisch in den Auftrag übernommen und im Protokoll vermerkt.
- 🚀 **Neu**: Direkte Kundenverwaltung im Auswahl-Modal – im `CustomerPickerModal` können neue Kontakte direkt per `+ Neuer Kontakt` angelegt und bestehende Kontakte inline per Stift-Icon bearbeitet werden, inklusive sichtbarer Stundensatz-Plakette.
- 🔄 **Verbessert**: Stundensatz-Synchronisation & Status-Plakette – in der Ticket-Detailansicht zeigt eine Plakette den hinterlegten Kunden-Stundensatz mit Ein-Klick-Übernahme (`Übernehmen`).

## 🌟 Version 22.6.8 (Kunden-Modal Dark-Mode Kontrast & Namensanzeige)
- 🛠️ **Behoben**: Kontrastfehler im Kunden-Auswahlfenster behoben – im Dark-Mode erschienen Kunden-Karten fälschlicherweise mit weißem Hintergrund und weißer Schrift. Dies wurde durch standardkonforme Dark-Mode-Klassen behoben (`dark:bg-slate-800/90`), sodass Name, Firma und Details wieder kontrastreich und gestochen scharf lesbar sind.
- 🔄 **Verbessert**: Robuste Namensanzeige mit automatischer Fallback-Kette – falls Kontakte nur einen Firmennamen oder keine Namenseingabe haben, wird der Firmenname sauber anstelle einer leeren Textzeile dargestellt.
- 🛠️ **Behoben**: Systemweite Bereinigung fehlerhafter Dark-Mode-Farbklassen in Studio-Drawer, Produktetiketten-Modal, Dashboard und Befehlspalette.

## 🌟 Version 22.6.7 (Kunden-Auswahlmodal, CRM-Autofill & Logbuch-Fokus)
- 🚀 **Neu**: Modales Kunden-Auswahlfenster (`CustomerPickerModal`) – schnelles Suchen und Filtern auch bei Hunderten oder Tausenden von Kundenkontakten mit Live-Textsuche (Name, Firma, E-Mail, Telefon, Stadt) und Typ-Filtern (Alle, Kunden, Lieferanten).
- 🛠️ **Behoben**: Zuverlässige Kunden-Übernahme – beim Auswählen eines Kontakts werden alle Kundendaten (`contact_name`, `contact_email`, `contact_phone`, `contact_company`) sofort vollständig und fehlerfrei in das Ticket übernommen.
- 🔄 **Verbessert**: Saubere leere Vorlage beim Erstellen neuer Tickets – kein ungewolltes automatisches Vorauswählen des ersten Adressbuch-Kontakts mehr.
- 🔄 **Verbessert**: Standardmäßig ausgeblendetes Logbuch / Chatter (`isChatterVisible: false`) – der gesamte Bildschirm steht sofort für das Ticket-Formular zur Verfügung. Mit einem Klick in der oberen Aktionsleiste kann das Logbuch jederzeit eingeblendet werden (inklusive Zähler-Badge für ungelesene Aktivitäten).
- 🚀 **Neu**: Elegante Kunden-Statuskarte im Ticket mit Initialen-Avatar, Unternehmens-Tag, 1-Klick-Button zum Kundenwechseln und Trennen der Verknüpfung sowie optionalem Schnell-Dropdown.
- 🛠️ **Behoben**: Vollständige viersprachige Lokalisierung (DE, EN, FR, ES) über `src/lib/i18n.ts` für alle neuen Such-, Auswahl- und Filter-Elemente.

## 🌟 Version 22.6.6 (Zahlungs-Datenschutz, Maskierte Kartennummern & PC-Terminal-Workflow)
- 🚀 **Neu**: Schutz sensibler Kartendaten – Kartennummern werden über alle Kartentypen hinweg (Girocard, Visa, Mastercard, Apple/Google Pay) durchgängig mit Sternchen maskiert (`4532 **** **** ****`). Das Umschalten des Kartentyps behält die Maskierung verlässlich bei.
- 🔄 **Verbessert**: Transparente PC-Terminal-Führung – verständliche Erklärung im Zahlungsfenster, warum PCs keinen eigenen Kartenschlitz besitzen und wie EC-Terminals (z. B. via ZVT über LAN/WLAN oder SumUp) angebunden werden.
- 🛠️ **Behoben**: Warnhinweis und Direkt-Absprung, wenn Kartenzahlung in den Einstellungen noch nicht aktiviert ist (`Kartenterminal (Erst in Settings machen!)`) mit Ein-Klick-Button direkt in den Einstellungs-Tab.
- 🚀 **Neu**: PCI-DSS-Hinweis zur Kunden-PIN – PIN-Eingabe erfolgt ausschließlich am Kundenterminal und wird niemals auf dem PC abgefragt oder gespeichert.
- 🔄 **Verbessert**: Stundenerfassung im Support-Modul – Zeiteinträge können direkt in der Tabelle inline editiert werden (Datum, Mitarbeiter, Tätigkeitsbeschreibung, Stunden).
- 🚀 **Neu**: "Solo-Modus / Keine Teams" im Support-Modul für Einzelanwender zur Reduzierung von Team- und Mitarbeiter-Overhead.
- 🛠️ **Behoben**: Viersprachige Lokalisierung (DE, EN, FR, ES) über `src/lib/i18n.ts` für alle neuen Zahlungs-, Terminal- und Support-Meldungen.

## 🌟 Version 22.6.5 (Support-Workspace-Redesign, Verschiebbare Spalten & Überlagerungs-Korrektur)
- 🚀 **Neu**: Stufenlos verschiebbarer Spalten-Trenner (Splitter) per Maus-Drag zwischen Ticket-Formular und dem internen Logbuch/Notizen-Bereich mit visueller Griffleiste und automatischer Speicherung der gewählten Wunschbreite in LocalStorage.
- 🛠️ **Behoben**: Überlagerung von E-Mail- und Telefon-Eingabefeldern im Ticket behoben – strukturierte, großzügige Zeilen mit Icons (`Mail`, `Telefon`, `Unternehmen`) und direkten Aktionsbuttons (`mailto:` für E-Mail-Entwurf und `tel:` für Direktanruf).
- 🔄 **Verbessert**: Zweistufige Kopfleiste im Ticket mit vollständiger horizontaler Status-Pipeline (ohne abgeschnittene Texte), aktuellem Status-Indikator mit pulsierendem Live-Punkt und klaren Aktions-Schaltflächen (Rechnung erstellen, Ticket abschließen / wiedereröffnen, Logbuch umschalten, Löschen).
- 🔄 **Verbessert**: Überflüssigen "SOCDOF standard"-Zusatz aus Untertiteln und interne Beschriftungen aus der Oberfläche und dem Chatter-Header entfernt für ein klares, aufgeräumtes Design.
- 🛠️ **Behoben**: Vollständige viersprachige Synchronisation (DE, EN, FR, ES) via `src/lib/i18n.ts` für alle neuen Status-, Splitter- und Feldanzeigen.
